...
func Get2(date time.Time, mealType string, details bool) (Meal, error) {
	...
	filter := expression.Name("date").Equal(expression.Value(date)).And(
		expression.Name("mealType").Equal(expression.Value(mealType)),
	)

	expr, err := expression.NewBuilder().WithFilter(filter).Build()
	if err != nil {
		fmt.Println("error building expression: ", expr)
		return Meal{}, err
	}
	...
	if err != nil {
		fmt.Println("Error while calling scan", err)
		return Meal{}, err
	}
	m := Meal{}
	for _, i := range result.Items {
		...
		if err != nil {
			fmt.Println("Unmashalling error: ", err)
			return m, err
		}
	}
	return m, nil
